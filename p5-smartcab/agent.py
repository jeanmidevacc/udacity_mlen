...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing :
            ...
            # a.epsilon=np.cos(0.5*trial)
            # self.epsilon = np.exp(-0.005 * trial)
            # a.epsilon = 1/(trial*trial)

            #self.epsilon-=0.05

            #self.epsilon=np.exp(-0.002 * self.trial)
            self.epsilon=1.0/(2*self.trial)
            #self.epsilon=math.fabs(math.cos(self.alpha * self.trial))
            #self.epsilon=np.abs(np.cos(self.alpha*self.trial))
            #self.epsilon=np.power(0.88,self.trial)
    ...
    def build_state(self):
        ...
        # state = 'deadline: {}, light: {}, left: {}, oncoming: {}, next_waypoint: {}'. \
